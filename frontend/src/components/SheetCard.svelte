<script lang="ts">
  ...
  const { sheet, selectedPieceId, pieceIndexes, onSelect }: Props = $props()

  const l10n = useL10n()
  const t = l10n.t
  const scale = $derived(sheetScale(sheet))
  const svgWidth = $derived(sheet.width * scale)
  const svgHeight = $derived(sheet.height * scale)
  const titleId = $derived(`sheet-title-${sheet.index}`)

  function pieceIndex(id: string): number {
    return pieceIndexes[id] ?? 0
  }
  ...
  function onPieceKeydown(event: KeyboardEvent, id: string) {
...
<article class="sheet-card">
  ...
  <div class="sheet-svg-wrap" id={`sheet-svg-${sheet.index}`}>
    ...
    >
      ...
      {#each grainLines(svgHeight) as gy, gi (`grain-${gi}`)}
        <line
          x1="0"
          y1={gy.toFixed(1)}
      ...
      {#each sheet.placedPieces as piece, piecePosition (`${piece.source.id}-${piecePosition}`)}
        <rect
          ...
          onclick={() => selectPiece(piece.source.id)}
          onkeydown={(event: KeyboardEvent) => onPieceKeydown(event, piece.source.id)}
        />
        ...
        {#if piece.width * scale > 40 && piece.height * scale > 22}
          {#if piece.source.label?.trim()}
            <text
