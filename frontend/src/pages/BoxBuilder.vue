...
const svgScale = (pw: number, ph: number) => G.svgScale(pw, ph)
...
// ── 3D contour generators (thin wrappers over src/box/geometry.ts) ───────────
...
function dlPiece(name: string, path: string, pw: number, ph: number, xOff = 0) {
  downloadSvg(name, G.wrapCutSvg(path, pw, ph, xOff))
}
