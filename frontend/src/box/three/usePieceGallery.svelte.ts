 ...
 *
 * Svelte 5 runes port: holds no reactive state of its own; it reads the
 * model's reactive properties directly. Identical Three.js logic to the Vue
 * composable.
 */
import * as THREE from 'three'
import { TrackballControls } from 'three/addons/controls/TrackballControls.js'
import { buildPanel, clearGroup, type PanelData } from './panelMesh'
import type { BoxModel } from '@/box/useBoxModel.svelte'
...
export function usePieceGallery(model: BoxModel) {
  ...
  function rebuildAllPieces() {
    ...
    for (let i = 0; i < pieces.length; i++) {
      ...
      if (gp.id === 'side') {
        ...
        const hh = model.horizHoles3D(0, model.TopD, Math.max(Bevel, 0))
        panels.push({ c: model.horizPts3D(0, model.TopD, Math.max(Bevel, 0)), n: [0, 0, 1], t: thick, col: '#27ae60', ec: '#1e8449', h: hh.length > 0 ? hh : undefined })
      } else if (gp.id === 'bot') {
        const hh = model.horizHoles3D(0, model.BotD, Math.max(-Bevel, 0))
        panels.push({ c: model.horizPts3D(0, model.BotD, Math.max(-Bevel, 0)), n: [0, 0, 1], t: thick, col: '#1abc9c', ec: '#148f77', h: hh.length > 0 ? hh : undefined })
      } else if (gp.id === 'back') {
        ...
        const sc2 = Bevel !== 0 ? model.shelfColor(si) : '#e67e22'
        const sec = Bevel !== 0 ? model.shelfEdgeColor(si) : '#ca6f1e'
        panels.push({ c: model.shelfPts3D(0, sd, sOff), n: [0, 0, 1], t: thick, col: sc2, ec: sec })
      }
  ...
  function pieceAngle(idx: number): number {
    return -(idx / model.galPieces.length) * Math.PI * 2
  }
