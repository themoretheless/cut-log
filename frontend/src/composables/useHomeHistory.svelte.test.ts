...
import { useHomeHistory } from './useHomeHistory.svelte'
...
function makeHistory(options: Parameters<typeof useHomeHistory>[0]) {
  let history!: ReturnType<typeof useHomeHistory>
  const stop = $effect.root(() => {
    history = useHomeHistory(options)
    return () => {}
  })
  return { get history() { return history }, stop }
}
...
describe('useHomeHistory', () => {
  it('coalesces edits and restores undo and redo snapshots', async () => {
    ...
    const { history, stop } = makeHistory({
      capture: () => current,
      apply,
      persist,
      onRestore,
  ...
  it('cancels a pending record when disposed', () => {
    ...
    history.record()
    expect(history.canUndo).toBe(true)
    // Svelte port: teardown is explicit (dispose) instead of Vue's scope.stop().
    history.dispose()
    vi.advanceTimersByTime(10)
    expect(history.canUndo).toBe(false)
    stop()
  ...
  it('keeps separate named actions as separate undo steps', () => {
    ...
    expect(history.lastRecordedAction).toBe('sheet.preset')
    expect(history.undo()).toBe(true)
    expect(current.sheetWidth).toBe(2500)
    expect(history.undo()).toBe(true)
    expect(current.sheetWidth).toBe(2440)
