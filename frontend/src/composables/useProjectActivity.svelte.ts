...
} from './useProjectSnapshots.svelte'
...
export function useProjectActivity(options: UseProjectActivityOptions) {
  ...
  const state = $state({
    operationLog: [] as OperationEntry[],
    operationQuery: '',
    snapshotCompare: null as SnapshotComparison | null,
  })
  ...
  const filteredOperationLog = $derived.by(() => {
    const query = state.operationQuery.trim().toLocaleLowerCase()
    if (!query) return state.operationLog
    return state.operationLog.filter(entry =>
  ...
  function saveOperationLogNow() {
    try {
      storage.setItem(OPERATION_LOG_KEY, JSON.stringify(state.operationLog))
  ...
  function loadOperationLog() {
    ...
    try {
      ...
      state.operationLog = Array.isArray(parsed)
        ? parsed
  ...
  function recordOperation(label: string, detail = '') {
    ...
    }, ...state.operationLog].slice(0, OPERATION_LOG_LIMIT)
    saveOperationLogNow()
  }
  ...
  function compareSnapshot(snapshot: ProjectSnapshot) {
    state.snapshotCompare = buildComparison(snapshot)
  ...
  function saveSnapshot(): ProjectActivitySnapshotResult {
    ...
    const result = snapshotStore.save(
      ...
      `${options.translate('snapshot.default_name')} ${snapshotStore.snapshots.length + 1}`,
    )
  ...
  return {
    get operationLog() { return state.operationLog },
    get operationQuery() { return state.operationQuery },
    set operationQuery(value: string) { state.operationQuery = value },
    get filteredOperationLog() { return filteredOperationLog },
    get snapshotCompare() { return state.snapshotCompare },
    get snapshots() { return snapshotStore.snapshots },
    get snapshotName() { return snapshotStore.name },
    set snapshotName(value: string) { snapshotStore.name = value },
    load,
    dispose,
    recordOperation,
    clearOperationLog,
