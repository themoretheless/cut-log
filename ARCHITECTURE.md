...
- `frontend/src/pages/Home.vue` is a ~958-line god component. Its orchestration
  (localStorage persistence, snapshot undo/redo, share links, bulk import,
  CSV/SVG/DXF/print export, drag-reorder, selection, keyboard shortcuts, costing,
  and ~130 lines of inline sheet SVG) lives in the component, so almost none of
  it is unit-tested.
- `frontend/src/box/useBoxModel.ts` entangles three reasons-to-change: pure
  geometry wrappers (good), i18n label string-matching (`label.startsWith(t(...))`,
  which silently breaks if a translation changes), and hardcoded color palettes.
- The inline sheet SVG in `Home.vue` renders piece labels without XML-escaping,
  while `lib/exportLayout.ts` escapes the same labels. Same logic, two
  implementations, divergent safety.
- The three.js scenes (`box/three/useAssemblyScene.ts`, `usePieceGallery.ts`)
  dispose the renderer but never traverse the scene graph, so per-mesh geometry
  and materials leak on every box-parameter rebuild (even though
  `panelMesh.ts` already exports a `disposeObj()`).
- Smaller duplication: color palettes split across `helpers/svg.ts` and
  `useBoxModel.ts`; `downloadFile`/`downloadSvg` duplicated per page; `jspdf`
  and `svg2pdf.js` are declared in `package.json` but never imported.

What is already good and should be preserved: the pure-lib + co-located-test
pattern (`homeState`, `shareLink`, `parsePieceList`, `history`, `costSummary`,
`piecesCsv`, `pieceOps`, `exportLayout`, `validatePiece`), the wasm adapter
boundary, the box geometry as a single source of truth feeding both renderers,
and the l10n key-parity guard.
...
| # | Lens | Core finding |
|---|------|--------------|
| 1 | SRP / god-components | `Home.vue` mixes ~15-17 concerns; the box scenes conflate lifecycle, animation, geometry, and interaction. |
| 2 | Coupling and dependency direction | `useBoxModel` couples geometry + i18n + color; persistence and i18n leak across layers; piece identity depends on translated strings. |
| 3 | Cohesion and module boundaries | `Home.vue` groups things that change for different reasons; boundaries should be drawn by reason-to-change (persistence, history, share, import, export, selection, drag, keyboard). |
| 4 | Layering | The pure-lib to composable to component stack is sound; the violations are page-local (inline SVG string work, orchestration in the page). |
| 5 | Testability and seams | The pure libs are tested; the ~455 lines of `Home.vue` script are not. Extract composables at clear seams so ~90% becomes testable without a DOM. |
| 6 | State and data flow | No single owner for project state; refs, watchers, history, and persistence interleave with five entry points all calling `applyState`. |
| 7 | DRY and reuse | Duplicated palettes, download helpers, SVG scaling, toast, and badge/label rendering; the on-screen SVG and the export SVG diverge. |
| 8 | Abstraction quality | Some primitive obsession (bare param refs, raw SVG coordinates); a few leaky abstractions (color validation, color cycling spread across files). |
...
| Composables (`composables/`, `box/`, `box/three/`) | Vue-reactive glue: wrap pure `lib` functions as refs/computed and own effects (watchers, timers, listeners, localStorage, clipboard, WebGL lifecycle). | May import `lib/`, `services/`, `stores/l10n`, and `vue`. `t()` is injected as a parameter into composables that should stay testable, not imported directly. Leaf composables do not import the page-level aggregator. |
...
| `composables/usePieceList.ts` | Piece CRUD + color allocation: add, import, remove, duplicate, clear, reorder. Owns `colorIdx`. | `lib/pieceOps`, `lib/parsePieceList`, `lib/palette`, `services/types`, `vue` |
...
the version-bump CI gate, and is verified before merge.
...
- Steps: remove `jspdf` and `svg2pdf.js` from `package.json` (no `src` imports);
  add `lib/downloadFile.ts` (+ test) and replace the inline wrappers in both pages.
...
- Verified by: vitest incl. the new test, build, one SVG-export smoke per page,
  no `jspdf`/`svg2pdf` in the bundle.

### Phase 1 - Consolidate palette and fix the SVG escape bug
- Goal: one color source and a tested sheet-SVG builder that escapes labels,
  closing the preview-vs-export divergence.
- Steps: add `lib/palette.ts` (+ tests) and route existing usages through it; add
  `lib/sheetSvg.ts` (+ tests covering a label with `<` and `&`); introduce
  `components/SheetCard.vue` and replace the inline SVG block in `Home.vue`.
- Risk: medium (the inline SVG is visually load-bearing; the escape change is a
  deliberate, correct fix).
...
### Phase 3 - Extract Home persistence and history composables
- Goal: make the most fragile, untested orchestration (localStorage timing,
  snapshot undo with the restoring guard) testable and isolated.
- Steps: confirm `lib/homeState` and `lib/history` cover the relied-on semantics,
  add cases if thin; add `composables/useHomeProject.ts` and
  `composables/useHomeHistory.ts`; rewire `Home.vue`, keeping the single watcher
  that calls save + record.
...
- Verified by: a focused history test (record, undo, redo, restore does not
  self-record); manual edit/undo/redo/reload/open-share-link sequence.

### Phase 4 - Extract the piece-list composable
- Goal: centralize piece CRUD, color allocation, import, duplicate, and reorder.
- Steps: move those into `composables/usePieceList.ts` delegating to the existing
  `lib` modules; keep drag-state refs in the page as UI transients, but the
  reorder mutation calls the composable.
- Risk: medium (color cycling, duplicate-after-source order, reorder index math).
- Verified by: existing `pieceOps` tests plus a reorder test if that logic moves to
  `lib`; manual add/import/duplicate/drag checks.

### Phase 5 - Decouple box geometry from i18n and color
- Goal: remove label string-matching and hardcoded palettes from `useBoxModel`.
- Steps: add `box/usePieceCatalog.ts` with a `PieceId` enum and id-keyed specs;
  refactor `allPieces`/`galPieces`/`pieceData` to consume the catalog by id; leave
  `geometry.ts` untouched.
...
- Steps: make both scenes' `dispose()` traverse the graph via `panelMesh.disposeObj`
  (small, high-value, can ship alone); only then, if still worth it, extract
  `box/three/useThreeScene.ts` for the shared boilerplate.
...
1. Extracting `Home.vue` persistence + undo/redo into composables (Phase 3): the highest risk-reduction per line moved, turning the most fragile untested code into isolated, testable units.
2. A tested `lib/sheetSvg.ts` behind `SheetCard.vue` (Phase 1): removes the largest template block and fixes a real XML-escape bug.
3. Decoupling box geometry from i18n string-matching (Phase 5): eliminates a latent bug where changing a translation silently breaks piece matching.
...
pass. Treat this as a living document: update it as phases land.
